...
type Task = { id: string; title: string; dueDate: Date | string | null; completed: boolean };

export function CustomerNotesTasks({ customerId, notes, tasks }: { customerId: string; notes: Note[]; tasks: Task[] }) {
  ...
  async function addTask(e: React.FormEvent) {
    ...
    await fetch("/api/tasks", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ customerId, title: taskTitle, dueDate: taskDue || null })
    });
    setTaskTitle("");
    setTaskDue("");
    setBusy(false);
