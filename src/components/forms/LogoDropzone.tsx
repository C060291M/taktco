...
export function LogoDropzone({
  ...
  async function handleFile(file: File) {
    ...
    // Local preview shows instantly regardless of where the file ends up.
    ...
    setUploading(true);
    try {
      const url = await uploadFileSmart(file, "logos");
    ...
    <div>
      ...
      {fileName && (
        <div className="flex items-center justify-between mt-2">
          <span className="text-xs text-graphite-400 truncate">{uploading ? "Uploading..." : fileName}</span>
