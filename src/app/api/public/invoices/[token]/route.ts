...
import { resolvePaymentAmount, recordInvoicePayment, totalPaidOnInvoice, PaymentChoice } from "@/lib/invoicePayments";
...
export async function GET(_req: NextRequest, { params }: { params: { token: string } }) {
  ...
  return NextResponse.json({
    ...
    company: {
      ...
      brandAccentColor: invoice.company.brandAccentColor,
      timeZone: invoice.company.timeZone,
      defaultDepositPercent: invoice.company.defaultDepositPercent
    },
...
// hosted Checkout Session for the resolved amount (deposit/full/remaining)
// and returns its URL - the actual PAID/PARTIALLY_PAID transition happens
// later, from the webhook, once Stripe confirms the charge, never here.
// Fallback: the original local dev stub, records the payment immediately.
//
// Legacy DEPOSIT/FINAL_BALANCE invoices (kind !== STANDARD) keep their old
// always-pay-the-full-invoice-amount behavior untouched.
export async function POST(req: NextRequest, { params }: { params: { token: string } }) {
  ...
  let amountToCharge = Number(invoice.amount);
  if (invoice.kind === "STANDARD") {
    ...
    try {
      amountToCharge = await resolvePaymentAmount(invoice.id, invoice.companyId, choice);
    } catch (err) {
      return NextResponse.json({ error: err instanceof Error ? err.message : "Could not resolve payment amount." }, { status: 400 });
    }
  }
  ...
  try {
    const updated = await recordInvoicePayment({ invoiceId: invoice.id, companyId: invoice.companyId, amount: amountToCharge, method: "manual" });
    return NextResponse.json({ status: updated.status });
  } catch (err) {
