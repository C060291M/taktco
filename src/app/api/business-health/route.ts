...
// A real 0-100 score, not a black box - every component below is disclosed
// to the client with its own sub-score and the real numbers behind it,
// same transparency discipline as the Insights engine's "why am I seeing
// this?". Nothing here is an AI guess; every number is a direct database
// calculation.
//
// 8 weighted components (100 pts total). Two things the spec asked for are
// deliberately NOT folded into the score itself: "overdue tasks" is surfaced
// as a separate actionable insight instead (a task backlog is an operational
// nudge, not really a "how healthy is the business" signal), and "pipeline
// value" is shown as context, not scored (a bigger pipeline isn't
// inherently healthier - it's informational). AI credit usage is omitted
// entirely, as the original recommendation marked it optional and it's a
// software-usage metric, not a business-health one.
export async function GET() {
  ...
  // 2. Gross margin vs target (15 pts) - from recent approved estimates that have cost data
  let marginScore = 7.5;
  ...
  if (withCostData.length > 0) {
    ...
    const target = ctx.company.targetMarginPercent ? Number(ctx.company.targetMarginPercent) : 20;
  ...
  // 3. Outstanding invoices health (15 pts)
  ...
  // 4. Average payment time (10 pts) - days from invoice issue to first payment
  ...
  if (paidInvoicesWithDates.length > 0) {
    ...
    if (days.length > 0) {
      ...
      const dueDays = ctx.company.defaultInvoiceDueDays || 14;
  ...
  // 7. Project completion rate (10 pts) - completed on or before target date
  ...
  // Real, threshold-based actionable insights - same honest-rules discipline
  // as the Insights engine, not AI-generated commentary.
